package com.dev.dzikirkita.ui.jadwal_shalat
...
import com.dev.dzikirkita.data.remote.retrofit.ApiConfig
        ...
        client.enqueue(object : Callback<JadwalShalatResponse> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    val responseBody = response.body()
                    if (responseBody != null) {
                        _setJadwalRespone.postValue(responseBody)
                    } else {
                        _isError.postValue("Konten tidak tersedia")
                    }
                } else {
                    Log.e("HadisViewModel", "onFailure: ${response.message()}")
                }
